...
def adicionar_meses(data_base: date, quantidade_meses: int) -> date:
    mes_total = (data_base.month - 1) + quantidade_meses
    ano = data_base.year + mes_total // 12
    mes = (mes_total % 12) + 1
    dia = min(data_base.day, ultimo_dia_do_mes(ano, mes))
    return date(ano, mes, dia)


def somar_frequencia(data_base: date, frequencia: str) -> date:
    frequencia = (frequencia or '').strip().lower()
    if frequencia == 'semanal':
        return data_base + timedelta(days=7)
    if frequencia == 'anual':
        return adicionar_meses(data_base, 12)
    return adicionar_meses(data_base, 1)
...
class BancoDados:
    ...
    def criar_tabelas(self):
        ...
        cur.execute(
            ...
            CREATE TABLE IF NOT EXISTS recurring_entries (
                ...
                category TEXT NOT NULL,
                amount REAL NOT NULL,
                frequency TEXT NOT NULL CHECK(frequency IN ('Mensal','Semanal','Anual')),
    ...
    def obter_transacoes(self, user_id, filtro_mes=None, filtro_tipo='Todos', filtro_categoria='Todas', busca_nome='', data_inicio=None, data_fim=None):
        cur = self.conn.cursor()
    ...
    def obter_resumo(self, user_id, filtro_mes=None, filtro_tipo='Todos', filtro_categoria='Todas', busca_nome='', data_inicio=None, data_fim=None):
        linhas = self.obter_transacoes(user_id, filtro_mes, filtro_tipo, filtro_categoria, busca_nome, data_inicio, data_fim)
    ...
    def transacao_ja_existe(self, user_id, tipo, nome, categoria, data_iso, valor, observacao=''):
        cur = self.conn.cursor()
        cur.execute(
            '''
            SELECT id FROM transactions
            WHERE user_id = ? AND type = ? AND name = ? AND category = ? AND date = ? AND amount = ? AND COALESCE(note, '') = ?
            LIMIT 1
            ''',
            (user_id, tipo, nome, categoria, data_iso, valor, observacao),
        )
        return cur.fetchone() is not None

    def adicionar_recorrencia(self, user_id, tipo, nome, categoria, valor, frequencia, data_inicio, data_fim=None, observacao=''):
        self.conn.execute(
            '''
            INSERT INTO recurring_entries (user_id, type, name, category, amount, frequency, start_date, end_date, note, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''',
            (user_id, tipo, nome, categoria, valor, frequencia, data_inicio, data_fim, observacao, datetime.now().isoformat())
        )
        self.conn.commit()

    def listar_recorrencias(self, user_id):
        cur = self.conn.cursor()
        cur.execute(
            '''
    ...
    def excluir_recorrencia(self, recorrencia_id, user_id):
        self.conn.execute('DELETE FROM recurring_entries WHERE id = ? AND user_id = ?', (recorrencia_id, user_id))
        self.conn.commit()

    def processar_recorrencias(self, user_id, data_limite=None):
        limite = data_limite or adicionar_meses(date.today(), 12)
        cur = self.conn.cursor()
        cur.execute(
            '''
            SELECT * FROM recurring_entries
        ...
        for rec in recorrencias:
            atual = datetime.strptime(rec['start_date'], '%Y-%m-%d').date()
            fim = datetime.strptime(rec['end_date'], '%Y-%m-%d').date() if rec['end_date'] else None
            limite_real = min(limite, fim) if fim else limite

            while atual <= limite_real:
                data_iso = atual.strftime('%Y-%m-%d')
                if not self.transacao_ja_existe(
                    user_id, rec['type'], rec['name'], rec['category'], data_iso, float(rec['amount']), rec['note'] or ''
                ):
                    self.adicionar_transacao(
                        user_id, rec['type'], rec['name'], rec['category'], data_iso, float(rec['amount']), rec['note'] or ''
                    )
                    inseridas += 1
                atual = somar_frequencia(atual, rec['frequency'])

        return inseridas

    def adicionar_plano_parcelado(self, user_id, nome, categoria, valor_total, parcelas, valor_parcela, primeira_data, cartao, observacao=''):
        cur = self.conn.cursor()
        cur.execute(
            '''
            INSERT INTO installment_plans (user_id, name, category, total_amount, installments, installment_amount, first_date, card_name, note, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''',
            (user_id, nome, categoria, valor_total, parcelas, valor_parcela, primeira_data, cartao, observacao, datetime.now().isoformat())
        )
        plano_id = cur.lastrowid

        data_base = datetime.strptime(primeira_data, '%Y-%m-%d').date()
        for indice in range(parcelas):
            data_parcela = adicionar_meses(data_base, indice).strftime('%Y-%m-%d')
            nota = f'{cartao} | Parcela {indice + 1}/{parcelas}'
            if observacao:
                nota = f'{nota} | {observacao}'
            self.adicionar_transacao(user_id, 'Despesa', nome, categoria, data_parcela, valor_parcela, nota)

        return plano_id

    def listar_parcelamentos(self, user_id):
        cur = self.conn.cursor()
        cur.execute(
            '''
    ...
    def excluir_plano_parcelado(self, plano_id, user_id):
        self.conn.execute('DELETE FROM installment_plans WHERE id = ? AND user_id = ?', (plano_id, user_id))
        self.conn.commit()
...
class JanelaMetas(tk.Toplevel):
    def __init__(self, master, banco: BancoDados, usuario, ao_atualizar):
        super().__init__(master)
        self.banco = banco
        ...
        self.montar_interface()
        self.carregar_meta_selecionada()
        self.listar_metas()
    ...
    def montar_interface(self):
        ...
        tk.Label(container, text='Metas do mês', font=('Segoe UI', 14, 'bold'), bg='white', fg='#1d2a57').pack(anchor='w', padx=14, pady=(14, 4))
        tk.Label(container, text='Escolha o período e cadastre metas de gasto e economia.', font=('Segoe UI', 9), bg='white', fg='#5f6f91').pack(anchor='w', padx=14, pady=(0, 10))

        topo = tk.Frame(container, bg='white')
        topo.pack(fill='x', padx=14, pady=(0, 10))

        esquerda = tk.Frame(topo, bg='#f8fbff', bd=1, relief='solid')
        esquerda.pack(side='left', fill='both', expand=True, padx=(0, 8))

        tk.Label(esquerda, text='Período', font=('Segoe UI', 10, 'bold'), bg='#f8fbff', fg='#1d2a57').pack(anchor='w', padx=12, pady=(12, 8))

        linha_periodo = tk.Frame(esquerda, bg='#f8fbff')
        linha_periodo.pack(fill='x', padx=12, pady=(0, 10))

        tk.Label(linha_periodo, text='Mês', font=('Segoe UI', 9, 'bold'), bg='#f8fbff').pack(side='left')
        meses = [f'{i:02d}' for i in range(1, 13)]
        ttk.Combobox(linha_periodo, textvariable=self.mes_var, values=meses, state='readonly', width=6).pack(side='left', padx=(6, 14))
        tk.Label(linha_periodo, text='Ano', font=('Segoe UI', 9, 'bold'), bg='#f8fbff').pack(side='left')
        ttk.Combobox(linha_periodo, textvariable=self.ano_var, values=[str(a) for a in range(date.today().year - 5, date.today().year + 6)], state='readonly', width=8).pack(side='left', padx=(6, 0))

        linha_gasto = tk.Frame(esquerda, bg='#f8fbff')
        linha_gasto.pack(fill='x', padx=12, pady=(0, 8))
        tk.Label(linha_gasto, text='Gasto máximo', font=('Segoe UI', 9, 'bold'), bg='#f8fbff').pack(anchor='w')
        tk.Entry(linha_gasto, textvariable=self.meta_gasto_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 0), ipady=5)

        linha_economia = tk.Frame(esquerda, bg='#f8fbff')
        linha_economia.pack(fill='x', padx=12, pady=(0, 8))
        tk.Label(linha_economia, text='Economia mínima', font=('Segoe UI', 9, 'bold'), bg='#f8fbff').pack(anchor='w')
        tk.Entry(linha_economia, textvariable=self.meta_economia_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 0), ipady=5)

        botoes = tk.Frame(esquerda, bg='#f8fbff')
        botoes.pack(fill='x', padx=12, pady=(4, 12))
        tk.Button(botoes, text='Carregar período', command=self.carregar_meta_selecionada, bg='#eef3ff', fg='#1d2a57', relief='flat').pack(side='left', fill='x', expand=True, padx=(0, 4), ipady=7)
        tk.Button(botoes, text='Salvar metas', command=self.salvar_metas, bg='#dbeafe', fg='#1e3a8a', relief='flat').pack(side='left', fill='x', expand=True, padx=4, ipady=7)
        tk.Button(botoes, text='Limpar', command=self.limpar_campos, bg='#f0f2f7', fg='#1d2a57', relief='flat').pack(side='left', fill='x', expand=True, padx=(4, 0), ipady=7)

        direita = tk.Frame(topo, bg='#f8fbff', bd=1, relief='solid')
        direita.pack(side='left', fill='both', expand=True, padx=(8, 0))

        tk.Label(direita, text='Metas cadastradas', font=('Segoe UI', 10, 'bold'), bg='#f8fbff', fg='#1d2a57').pack(anchor='w', padx=12, pady=(12, 8))

        colunas = ('periodo', 'gasto', 'economia')
        self.tree_metas = ttk.Treeview(direita, columns=colunas, show='headings', height=11)
        for col, titulo, largura in [('periodo', 'Período', 90), ('gasto', 'Gasto máximo', 140), ('economia', 'Economia mínima', 150)]:
            self.tree_metas.heading(col, text=titulo)
            self.tree_metas.column(col, width=largura, anchor='center')
        self.tree_metas.pack(fill='both', expand=True, padx=12, pady=(0, 12))
        self.tree_metas.bind('<<TreeviewSelect>>', self.carregar_meta_da_lista)
    ...
    def listar_metas(self):
        ...
        cur.execute(
            '''
            SELECT month_ref,
                   MAX(CASE WHEN goal_type = 'gasto_maximo' THEN amount END) AS gasto,
                   MAX(CASE WHEN goal_type = 'economia_minima' THEN amount END) AS economia
            FROM goals
            WHERE user_id = ?
            GROUP BY month_ref
        ...
        for linha in cur.fetchall():
            self.tree_metas.insert('', 'end', values=(
                linha['month_ref'],
                '' if linha['gasto'] is None else formatar_moeda(float(linha['gasto'])),
                '' if linha['economia'] is None else formatar_moeda(float(linha['economia']))
            ))

    def carregar_meta_selecionada(self):
    ...
    def carregar_meta_da_lista(self, _event=None):
        selecionados = self.tree_metas.selection()
        if not selecionados:
            return
        periodo = self.tree_metas.item(selecionados[0], 'values')[0]
    ...
    def salvar_metas(self):
        try:
            ...
            if self.meta_gasto_var.get().strip():
                self.banco.salvar_meta(self.usuario['id'], month_ref, 'gasto_maximo', float(self.meta_gasto_var.get().replace('.', '').replace(',', '.')))
            if self.meta_economia_var.get().strip():
                self.banco.salvar_meta(self.usuario['id'], month_ref, 'economia_minima', float(self.meta_economia_var.get().replace('.', '').replace(',', '.')))
            self.listar_metas()
            self.ao_atualizar()
            messagebox.showinfo('Sucesso', 'Metas salvas com sucesso.', parent=self)
        except Exception:
            messagebox.showwarning('Aviso', 'Informe valores válidos para as metas.', parent=self)

class JanelaTransacoesRecorrentes(tk.Toplevel):
    def __init__(self, master, banco: BancoDados, usuario, ao_atualizar=None):
        super().__init__(master)
        self.title('Transações recorrentes')
        ...
        self.categoria_var = tk.StringVar(value=CATEGORIAS_RECEITA[0])
        self.nome_var = tk.StringVar()
        self.valor_var = tk.StringVar()
        self.frequencia_var = tk.StringVar(value='Mensal')
        self.data_inicio_var = tk.StringVar(value=datetime.now().strftime('%d/%m/%Y'))
    ...
    def montar_interface(self):
        ...
        tk.Label(topo, text='Transações recorrentes', font=('Segoe UI', 14, 'bold'), bg='white', fg='#1d2a57').pack(anchor='w')
        tk.Label(topo, text='Cadastre lançamentos contínuos como salário, VA/VR e contas fixas.', font=('Segoe UI', 9), bg='white', fg='#5f6f91').pack(anchor='w', pady=(2, 0))

        corpo = tk.Frame(container, bg='#eef2f8')
        ...
        campos = tk.Frame(esquerda, bg='#f8fbff')
        campos.pack(fill='both', expand=True, padx=12, pady=12)

        tk.Label(campos, text='Tipo', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        tipo_box = tk.Frame(campos, bg='#f8fbff')
        tipo_box.pack(fill='x', pady=(4, 10))
        tk.Radiobutton(tipo_box, text='Receita', variable=self.tipo_var, value='Receita', command=self.atualizar_categorias, bg='#f8fbff').pack(side='left')
        tk.Radiobutton(tipo_box, text='Despesa', variable=self.tipo_var, value='Despesa', command=self.atualizar_categorias, bg='#f8fbff').pack(side='left', padx=(12, 0))

        for texto, var in [('Categoria', self.categoria_var), ('Nome', self.nome_var), ('Valor', self.valor_var)]:
            tk.Label(campos, text=texto, font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
            if texto == 'Categoria':
                self.combo_categoria = ttk.Combobox(campos, textvariable=var, state='readonly', font=('Segoe UI', 10))
                self.combo_categoria.pack(fill='x', pady=(4, 10), ipady=4)
            else:
                tk.Entry(campos, textvariable=var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 10), ipady=6)

        tk.Label(campos, text='Frequência', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        ttk.Combobox(campos, textvariable=self.frequencia_var, values=['Mensal', 'Semanal', 'Anual'], state='readonly', font=('Segoe UI', 10)).pack(fill='x', pady=(4, 10), ipady=4)

        linha_datas = tk.Frame(campos, bg='#f8fbff')
        linha_datas.pack(fill='x', pady=(0, 10))
        for idx, (titulo, var, cmd) in enumerate([('Início', self.data_inicio_var, lambda: CalendarioMini(self, self.data_inicio_var.get(), self.data_inicio_var.set)),
                                                  ('Fim (opcional)', self.data_fim_var, lambda: CalendarioMini(self, self.data_fim_var.get(), self.data_fim_var.set))]):
            bloco = tk.Frame(linha_datas, bg='#f8fbff')
            bloco.pack(side='left', fill='x', expand=True, padx=(0 if idx == 0 else 6, 6 if idx == 0 else 0))
            tk.Label(bloco, text=titulo, font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
            sub = tk.Frame(bloco, bg='#f8fbff')
            sub.pack(fill='x', pady=(4, 0))
            tk.Entry(sub, textvariable=var, font=('Segoe UI', 10), relief='solid', bd=1).pack(side='left', fill='x', expand=True, ipady=6)
            tk.Button(sub, text='📅', command=cmd, bg='#eef3ff', relief='flat').pack(side='left', padx=(6, 0), ipadx=8, ipady=6)

        tk.Label(campos, text='Observação', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        tk.Entry(campos, textvariable=self.observacao_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 10), ipady=6)

        acoes = tk.Frame(campos, bg='#f8fbff')
        acoes.pack(fill='x', pady=(4, 0))
        tk.Button(acoes, text='Salvar recorrência', command=self.salvar, bg='#4a90e2', fg='white', relief='flat').pack(side='left', fill='x', expand=True, padx=(0, 6), ipady=8)
        tk.Button(acoes, text='Processar agora', command=self.processar_agora, bg='#daf0e5', fg='#125c2f', relief='flat').pack(side='left', fill='x', expand=True, padx=6, ipady=8)
        tk.Button(acoes, text='Excluir selecionada', command=self.excluir, bg='#fde2e1', fg='#8a1c1c', relief='flat').pack(side='left', fill='x', expand=True, padx=(6, 0), ipady=8)

        tk.Label(direita, text='Recorrências cadastradas', font=('Segoe UI', 11, 'bold'), bg='#f8fbff', fg='#1d2a57').pack(anchor='w', padx=12, pady=(12, 8))
        cols = ('id', 'tipo', 'nome', 'freq', 'inicio', 'fim', 'valor')
        self.tree = ttk.Treeview(direita, columns=cols, show='headings', height=14)
        for col, titulo, largura in [('id', 'ID', 50), ('tipo', 'Tipo', 80), ('nome', 'Nome', 180), ('freq', 'Frequência', 90), ('inicio', 'Início', 90), ('fim', 'Fim', 90), ('valor', 'Valor', 110)]:
            self.tree.heading(col, text=titulo)
            self.tree.column(col, width=largura, anchor='center' if col not in ('nome', 'valor') else ('w' if col=='nome' else 'e'))
    ...
    def preencher_lista(self):
        ...
        for linha in self.banco.listar_recorrencias(self.usuario['id']):
            self.tree.insert('', 'end', iid=str(linha['id']), values=(
                linha['id'], linha['type'], linha['name'], linha['frequency'],
                converter_data_para_br(linha['start_date']),
                converter_data_para_br(linha['end_date']) if linha['end_date'] else '—',
                formatar_moeda(linha['amount'])
            ))

    def salvar(self):
        try:
            data_inicio = converter_data_para_iso(self.data_inicio_var.get())
            data_fim = converter_data_para_iso(self.data_fim_var.get()) if self.data_fim_var.get().strip() else None
            valor = float(self.valor_var.get().replace('.', '').replace(',', '.'))
        except Exception:
            messagebox.showwarning('Aviso', 'Preencha datas e valor corretamente.', parent=self)
            return
        if not self.nome_var.get().strip():
            messagebox.showwarning('Aviso', 'Informe o nome da recorrência.', parent=self)
            return
        self.banco.adicionar_recorrencia(
            self.usuario['id'], self.tipo_var.get(), self.nome_var.get().strip(), self.categoria_var.get(),
            valor, self.frequencia_var.get(), data_inicio, data_fim, self.observacao_var.get().strip()
        )
        self.processar_agora(mostrar_msg=False)
        self.nome_var.set(''); self.valor_var.set(''); self.observacao_var.set('')
        self.preencher_lista()
        messagebox.showinfo('Sucesso', 'Transação recorrente cadastrada com sucesso.', parent=self)

    def processar_agora(self, mostrar_msg=True):
        inseridas = self.banco.processar_recorrencias(self.usuario['id'])
        if self.ao_atualizar:
            self.ao_atualizar()
        self.preencher_lista()
        if mostrar_msg:
            messagebox.showinfo('Processamento concluído', f'{inseridas} lançamento(s) recorrente(s) foram gerados.', parent=self)

    def excluir(self):
        sel = self.tree.selection()
        if not sel:
            messagebox.showwarning('Aviso', 'Selecione uma recorrência para excluir.', parent=self)
            return
        if not messagebox.askyesno('Excluir', 'Deseja excluir a recorrência selecionada?', parent=self):
            return
        self.banco.excluir_recorrencia(int(sel[0]), self.usuario['id'])
        self.preencher_lista()
        if self.ao_atualizar:
            self.ao_atualizar()


class JanelaParcelamentoCartao(tk.Toplevel):
    def __init__(self, master, banco: BancoDados, usuario, ao_atualizar=None):
        super().__init__(master)
        self.title('Parcelamento do cartão')
        self.geometry('980x560')
        self.configure(bg='#eef2f8')
        self.banco = banco
        self.usuario = usuario
        ...
        self.parcelas_var = tk.StringVar(value='2')
        self.valor_parcela_var = tk.StringVar()
        self.data_primeira_var = tk.StringVar(value=datetime.now().strftime('%d/%m/%Y'))
    ...
    def montar_interface(self):
        container = tk.Frame(self, bg='white', bd=1, relief='solid')
        container.pack(fill='both', expand=True, padx=12, pady=12)
        tk.Label(container, text='Parcelamento do cartão', font=('Segoe UI', 14, 'bold'), bg='white', fg='#1d2a57').pack(anchor='w', padx=14, pady=(14, 4))
        tk.Label(container, text='Cadastre compras parceladas e o sistema gera todas as parcelas automaticamente.', font=('Segoe UI', 9), bg='white', fg='#5f6f91').pack(anchor='w', padx=14, pady=(0, 10))
        ...
        def add_entry(rotulo, var):
            tk.Label(campos, text=rotulo, font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
            ent = tk.Entry(campos, textvariable=var, font=('Segoe UI', 10), relief='solid', bd=1)
            ent.pack(fill='x', pady=(4, 10), ipady=6)
            return ent

        add_entry('Nome da compra', self.nome_var)
        tk.Label(campos, text='Categoria', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        ttk.Combobox(campos, textvariable=self.categoria_var, values=CATEGORIAS_DESPESA, state='readonly', font=('Segoe UI', 10)).pack(fill='x', pady=(4, 10), ipady=4)
        add_entry('Valor total', self.valor_total_var)
        linha = tk.Frame(campos, bg='#f8fbff'); linha.pack(fill='x', pady=(0, 10))
        b1 = tk.Frame(linha, bg='#f8fbff'); b1.pack(side='left', fill='x', expand=True, padx=(0, 6))
        b2 = tk.Frame(linha, bg='#f8fbff'); b2.pack(side='left', fill='x', expand=True, padx=(6, 0))
        tk.Label(b1, text='Parcelas', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        tk.Entry(b1, textvariable=self.parcelas_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 0), ipady=6)
        tk.Label(b2, text='Valor por parcela', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w')
        tk.Entry(b2, textvariable=self.valor_parcela_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(fill='x', pady=(4, 0), ipady=6)

        tk.Label(campos, text='Primeira parcela', font=('Segoe UI', 10, 'bold'), bg='#f8fbff').pack(anchor='w', pady=(10, 0))
        ld = tk.Frame(campos, bg='#f8fbff'); ld.pack(fill='x', pady=(4, 10))
        tk.Entry(ld, textvariable=self.data_primeira_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(side='left', fill='x', expand=True, ipady=6)
        tk.Button(ld, text='📅', command=lambda: CalendarioMini(self, self.data_primeira_var.get(), self.data_primeira_var.set), bg='#eef3ff', relief='flat').pack(side='left', padx=(6, 0), ipadx=8, ipady=6)

        add_entry('Cartão', self.cartao_var)
        add_entry('Observação', self.observacao_var)

        acoes = tk.Frame(campos, bg='#f8fbff'); acoes.pack(fill='x', pady=(4, 0))
        tk.Button(acoes, text='Salvar parcelamento', command=self.salvar, bg='#4a90e2', fg='white', relief='flat').pack(side='left', fill='x', expand=True, padx=(0, 6), ipady=8)
        tk.Button(acoes, text='Excluir selecionado', command=self.excluir, bg='#fde2e1', fg='#8a1c1c', relief='flat').pack(side='left', fill='x', expand=True, padx=(6, 0), ipady=8)

        tk.Label(direita, text='Parcelamentos cadastrados', font=('Segoe UI', 11, 'bold'), bg='#f8fbff', fg='#1d2a57').pack(anchor='w', padx=12, pady=(12, 8))
        cols = ('id', 'nome', 'cartao', 'parcelas', 'primeira', 'valor')
        self.tree = ttk.Treeview(direita, columns=cols, show='headings', height=14)
        for col, titulo, largura in [('id', 'ID', 50), ('nome', 'Nome', 180), ('cartao', 'Cartão', 120), ('parcelas', 'Parcelas', 80), ('primeira', '1ª parcela', 100), ('valor', 'Valor total', 110)]:
            self.tree.heading(col, text=titulo)
            self.tree.column(col, width=largura, anchor='center' if col not in ('nome','valor') else ('w' if col=='nome' else 'e'))
        self.tree.pack(fill='both', expand=True, padx=12, pady=(0, 12))
    ...
    def preencher_lista(self):
        for item in self.tree.get_children():
            self.tree.delete(item)
        for linha in self.banco.listar_parcelamentos(self.usuario['id']):
            self.tree.insert('', 'end', iid=str(linha['id']), values=(
                linha['id'], linha['name'], linha['card_name'], f"{linha['installments']}x",
                converter_data_para_br(linha['first_date']), formatar_moeda(linha['total_amount'])
            ))

    def salvar(self):
        try:
            valor_total = float(self.valor_total_var.get().replace('.', '').replace(',', '.'))
            parcelas = int(self.parcelas_var.get())
            valor_parcela = float(self.valor_parcela_var.get().replace('.', '').replace(',', '.')) if self.valor_parcela_var.get().strip() else round(valor_total / parcelas, 2)
            primeira = converter_data_para_iso(self.data_primeira_var.get())
        except Exception:
            messagebox.showwarning('Aviso', 'Revise valor, parcelas e data.', parent=self)
            return
        if not self.nome_var.get().strip() or not self.cartao_var.get().strip() or parcelas < 2:
            messagebox.showwarning('Aviso', 'Informe nome, cartão e pelo menos 2 parcelas.', parent=self)
            return
        self.banco.adicionar_plano_parcelado(
            self.usuario['id'], self.nome_var.get().strip(), self.categoria_var.get(), valor_total, parcelas,
            valor_parcela, primeira, self.cartao_var.get().strip(), self.observacao_var.get().strip()
        )
        self.preencher_lista()
        if self.ao_atualizar:
            self.ao_atualizar()
        messagebox.showinfo('Sucesso', 'Parcelamento cadastrado e parcelas lançadas com sucesso.', parent=self)

    def excluir(self):
        sel = self.tree.selection()
        if not sel:
            messagebox.showwarning('Aviso', 'Selecione um parcelamento.', parent=self)
            return
        if not messagebox.askyesno('Excluir', 'Excluir apenas o plano cadastrado? As parcelas já lançadas permanecerão na tabela.', parent=self):
            return
        self.banco.excluir_plano_parcelado(int(sel[0]), self.usuario['id'])
        self.preencher_lista()
...
class TelaPrincipal(tk.Frame):
    def __init__(self, master, banco: BancoDados, usuario, ao_sair_conta):
        ...
        self.janela_recorrentes = None
        self.janela_parcelamento = None
        ...
        self.meta_gasto_var = tk.StringVar()
        self.meta_economia_var = tk.StringVar()
        self.data_inicio_filtro_var = tk.StringVar()
        self.data_fim_filtro_var = tk.StringVar()
        self.alerta_meta_var = tk.StringVar(value='Metas e alertas aparecerão aqui.')
        self.resumo_mensal_var = tk.StringVar(value='Resumo mensal em preparação...')

        self.banco.processar_recorrencias(self.usuario['id'])
        self.montar_interface()
    ...
    def montar_interface(self):
        ...
        menu_conta = tk.Menubutton(topo, text='Conta e banco ▾', bg='#ffffff', fg='#1d2a57', relief='flat')
        menu_conta.menu = tk.Menu(menu_conta, tearoff=0)
        menu_conta['menu'] = menu_conta.menu
        menu_conta.menu.add_command(label='Backup do banco', command=self.fazer_backup_banco)
        menu_conta.menu.add_command(label='Trocar usuário', command=self.confirmar_troca_usuario)
        menu_conta.menu.add_separator()
        menu_conta.menu.add_command(label='Sair do aplicativo', command=self.confirmar_saida_app)
        menu_conta.pack(side='right', padx=(8, 18), pady=12)
        ...
        tk.Button(
            ...
            command=self.abrir_janela_recorrentes,
            bg='#eef3ff',
            fg='#1d2a57',
            relief='flat',
            font=('Segoe UI', 10, 'bold')
        ...
        tk.Button(
            linha_botoes_topo,
            text='Parcelamento do cartão',
            command=self.abrir_janela_parcelamento,
            bg='#eef3ff',
            fg='#1d2a57',
            relief='flat',
            font=('Segoe UI', 10, 'bold')
        ...
        totais.pack(fill='x', padx=12, pady=(0, 8))
        self.card_receitas = self.criar_card_total(totais, 'Receitas', self.total_receitas_var, '#d8f3dc', '#1b5e20')
        self.card_receitas.pack(side='left', fill='x', expand=True, padx=(0, 6))
        self.card_despesas = self.criar_card_total(totais, 'Despesas', self.total_despesas_var, '#ffe5d9', '#9a3412')
        self.card_despesas.pack(side='left', fill='x', expand=True, padx=6)
        self.card_saldo = self.criar_card_total(totais, 'Saldo da conta', self.saldo_var, '#dbeafe', '#1e3a8a')
        self.card_saldo.pack(side='left', fill='x', expand=True, padx=(6, 0))

        faixa_resumo = tk.Frame(self, bg='#eef2f8')
        faixa_resumo.pack(fill='x', padx=12, pady=(0, 12))
        tk.Label(faixa_resumo, textvariable=self.resumo_mensal_var, font=('Segoe UI', 10, 'bold'), bg='#e8f0ff', fg='#1d2a57', anchor='w', relief='solid', bd=1).pack(side='left', fill='x', expand=True, ipady=8)
        tk.Label(faixa_resumo, textvariable=self.alerta_meta_var, font=('Segoe UI', 10, 'bold'), bg='#fff4d6', fg='#8a5a00', anchor='w', relief='solid', bd=1).pack(side='left', fill='x', expand=True, padx=(8, 0), ipady=8)
    ...
    def montar_formulario(self, parent):
        ...
        card_automacoes = tk.Frame(painel_lateral, bg='#f8fbff', bd=1, relief='solid', highlightbackground='#d7e6ff', highlightthickness=1)
        card_automacoes.pack(fill='x')

        tk.Label(card_automacoes, text='Automatizações', font=('Segoe UI', 10, 'bold'), bg='#f8fbff', fg='#1d2a57').pack(anchor='w', padx=10, pady=(10, 4))
        tk.Label(card_automacoes, text='Abra janelas separadas para metas, recorrências e parcelamentos.', font=('Segoe UI', 8), bg='#f8fbff', fg='#5f6f91', justify='left', wraplength=150).pack(anchor='w', padx=10, pady=(0, 8))

        tk.Button(card_automacoes, text='Metas do mês', command=self.abrir_janela_metas, bg='#eef3ff', fg='#1d2a57', relief='flat').pack(fill='x', padx=10, pady=(0, 8), ipady=8)
        tk.Button(card_automacoes, text='Transações recorrentes', command=self.abrir_janela_recorrentes, bg='#eef3ff', fg='#1d2a57', relief='flat').pack(fill='x', padx=10, pady=(0, 8), ipady=8)
        tk.Button(card_automacoes, text='Parcelamento do cartão', command=self.abrir_janela_parcelamento, bg='#eef3ff', fg='#1d2a57', relief='flat').pack(fill='x', padx=10, pady=(0, 10), ipady=8)
    ...
    def montar_tabela(self, parent):
        ...
        tk.Label(faixa_filtro, text='Data inicial:', font=('Segoe UI', 10, 'bold'), bg='white', fg='#1d2a57').grid(row=2, column=0, sticky='w', pady=(8, 0))
        linha_ini = tk.Frame(faixa_filtro, bg='white')
        linha_ini.grid(row=2, column=1, padx=(6, 10), sticky='ew', pady=(8, 0))
        tk.Entry(linha_ini, textvariable=self.data_inicio_filtro_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(side='left', fill='x', expand=True, ipady=4)
        tk.Button(linha_ini, text='📅', command=lambda: CalendarioMini(self, self.data_inicio_filtro_var.get(), self.data_inicio_filtro_var.set), bg='#eef3ff', relief='flat').pack(side='left', padx=(6, 0))

        tk.Label(faixa_filtro, text='Data final:', font=('Segoe UI', 10, 'bold'), bg='white', fg='#1d2a57').grid(row=2, column=2, sticky='w', pady=(8, 0))
        linha_fim = tk.Frame(faixa_filtro, bg='white')
        linha_fim.grid(row=2, column=3, padx=(6, 10), sticky='ew', pady=(8, 0))
        tk.Entry(linha_fim, textvariable=self.data_fim_filtro_var, font=('Segoe UI', 10), relief='solid', bd=1).pack(side='left', fill='x', expand=True, ipady=4)
        tk.Button(linha_fim, text='📅', command=lambda: CalendarioMini(self, self.data_fim_filtro_var.get(), self.data_fim_filtro_var.set), bg='#eef3ff', relief='flat').pack(side='left', padx=(6, 0))

        tk.Button(faixa_filtro, text='Aplicar período', command=self.atualizar_tudo, bg='#eef3ff', fg='#1d2a57', relief='flat').grid(row=2, column=4, sticky='ew', pady=(8, 0))
        tk.Button(faixa_filtro, text='Ver histórico completo', command=self.mostrar_historico_completo, bg='#f0f2f7', fg='#1d2a57', relief='flat').grid(row=2, column=5, sticky='ew', pady=(8, 0))
        tk.Label(faixa_filtro, textvariable=self.info_filtro_var, font=('Segoe UI', 9), bg='white', fg='#5f6f91').grid(row=0, column=6, rowspan=3, padx=(10, 0), sticky='e')

        faixa_filtro.columnconfigure(1, weight=1)
        faixa_filtro.columnconfigure(3, weight=1)
    ...
    def detectar_delimitador_csv(self, amostra: str) -> str:
        try:
    ...
    def normalizar_cabecalho_csv(self, texto: str) -> str:
        return ''.join(ch.lower() for ch in (texto or '').strip() if ch.isalnum())
    ...
    def importar_csv(self):
        ...
        try:
            with open(caminho, 'r', encoding='utf-8-sig', newline='') as arquivo:
                ...
                arquivo.seek(0)
                delimitador = self.detectar_delimitador_csv(amostra)
                leitor = csv.DictReader(arquivo, delimiter=delimitador)
                if not leitor.fieldnames:
                    messagebox.showwarning('Aviso', 'O CSV não possui cabeçalho.')
                    return

                mapa = {self.normalizar_cabecalho_csv(nome): nome for nome in leitor.fieldnames}

                def pegar(linha, opcoes):
                    for opcao in opcoes:
                        chave_real = mapa.get(opcao)
                        if chave_real:
                            return (linha.get(chave_real) or '').strip()
                    return ''
                ...
                for linha in leitor:
                    tipo = pegar(linha, ['tipo', 'tipodelancamento'])
                    nome = pegar(linha, ['nome', 'descricao', 'descricao', 'titulo'])
                    categoria = pegar(linha, ['categoria', 'category'])
                    data_txt = pegar(linha, ['data', 'datadotransacao', 'datalancamento'])
                    valor_txt = pegar(linha, ['valor', 'quantia', 'amount'])
                    observacao = pegar(linha, ['observacao', 'observações', 'obs', 'nota', 'note'])

                    if not tipo or not nome or not categoria or not data_txt or not valor_txt:
                        ignoradas += 1
                        continue

                    tipo_limpo = tipo.strip().lower()
                    if tipo_limpo in ('receita', 'r', 'entrada'):
                        tipo_final = 'Receita'
                    elif tipo_limpo in ('despesa', 'd', 'saida', 'saída'):
                        tipo_final = 'Despesa'
                    else:
                        ignoradas += 1
                        continue
                    ...
                    try:
                        if '/' in data_txt:
                            ...
                            data_iso = datetime.strptime(data_txt, '%Y-%m-%d').strftime('%Y-%m-%d')
                        valor = self.interpretar_valor(valor_txt)
                        if valor <= 0:
                            raise ValueError
                    except Exception:
                        ignoradas += 1
            ...
            if not transacoes:
                messagebox.showwarning('Aviso', 'Nenhuma linha válida foi encontrada para importar.')
                return

            self.banco.importar_transacoes_em_lote(self.usuario['id'], transacoes)
            self.atualizar_tudo()
            messagebox.showinfo('Importação concluída', f'{len(transacoes)} lançamento(s) importado(s) com sucesso.\nLinhas ignoradas: {ignoradas}')
        except Exception as e:
    ...
    def mostrar_historico_completo(self):
        ...
        self.data_inicio_filtro_var.set('')
        self.data_fim_filtro_var.set('')
        self.alterar_filtro_mes()

    def alterar_filtro_mes(self):
        atual = self.filtro_mes_var.get()
        periodo = []
        if self.data_inicio_filtro_var.get().strip():
            periodo.append(f'início {self.data_inicio_filtro_var.get().strip()}')
        if self.data_fim_filtro_var.get().strip():
            periodo.append(f'fim {self.data_fim_filtro_var.get().strip()}')
        complemento = (' | ' + ' | '.join(periodo)) if periodo else ''
        if atual == 'Histórico completo':
            self.info_filtro_var.set(f'Exibindo: histórico completo{complemento}')
        else:
            self.info_filtro_var.set(f'Exibindo apenas: {atual}{complemento}')
        self.carregar_metas_mes_atual()
        self.atualizar_tudo()
    ...
    def obter_datas_filtro_iso(self):
        data_inicio_iso = None
        data_fim_iso = None
        try:
            if self.data_inicio_filtro_var.get().strip():
                data_inicio_iso = converter_data_para_iso(self.data_inicio_filtro_var.get().strip())
            if self.data_fim_filtro_var.get().strip():
                data_fim_iso = converter_data_para_iso(self.data_fim_filtro_var.get().strip())
        except Exception:
            messagebox.showwarning('Aviso', 'Use o formato dd/mm/aaaa no filtro por período.')
            return None, None
        return data_inicio_iso, data_fim_iso

    def obter_linhas_filtradas(self):
        data_inicio_iso, data_fim_iso = self.obter_datas_filtro_iso()
    ...
    def salvar_transacao(self):
        ...
        eh_novo = self.id_selecionado is None
        ...
        self.limpar_formulario(manter_tipo=eh_novo)
        if eh_novo:
    ...
    def excluir_selecionado(self):
        ...
        self.limpar_formulario(manter_tipo=True)
        self.tipo_var.set(tipo_atual)
        self.atualizar_menu_categorias()
        self.atualizar_tudo()
    ...
    def atualizar_resumo(self):
        ...
        periodo = self.filtro_mes_var.get() if self.filtro_mes_var.get() != 'Histórico completo' else 'histórico completo'
        if self.data_inicio_filtro_var.get().strip() or self.data_fim_filtro_var.get().strip():
            trecho_periodo = f" | período: {self.data_inicio_filtro_var.get().strip() or '...'} até {self.data_fim_filtro_var.get().strip() or '...'}"
        else:
            trecho_periodo = ''
        self.resumo_mensal_var.set(
            f"Resumo mensal: {periodo} | lançamentos: {resumo['quantidade']} | saldo do filtro: {formatar_moeda(resumo['saldo'])}{trecho_periodo}"
        )
    ...
    def atualizar_dashboard(self):
        ...
        alerta = 'Metas e alertas aparecerão aqui.'
        bg_despesas = '#ffe5d9'
        bg_saldo = '#dbeafe'

        if metas['gasto_maximo'] is not None and resumo['despesas'] > metas['gasto_maximo']:
            excedente = resumo['despesas'] - metas['gasto_maximo']
            alerta = f'⚠ Atenção: você passou {formatar_moeda(excedente)} da meta de gastos.'
            bg_despesas = '#ffd6d6'

        if metas['economia_minima'] is not None and resumo['saldo'] < metas['economia_minima']:
            falta = metas['economia_minima'] - resumo['saldo']
            alerta = f'⚠ Economia abaixo da meta em {formatar_moeda(falta)}.'
            bg_saldo = '#ffe3c4'

        self.alerta_meta_var.set(alerta)
        self.aplicar_cor_card(self.card_despesas, bg_despesas)
        self.aplicar_cor_card(self.card_saldo, bg_saldo)
        self.aplicar_cor_card(self.card_receitas, '#d8f3dc')
    ...
    def aplicar_cor_card(self, card, cor):
        card.config(bg=cor)
    ...
    def abrir_janela_metas(self):
        if self.janela_metas and self.janela_metas.winfo_exists():
            ...
            return
        self.janela_metas = JanelaMetas(self, self.banco, self.usuario, self.atualizar_tudo)

    def abrir_janela_recorrentes(self):
        if self.janela_recorrentes and self.janela_recorrentes.winfo_exists():
            self.janela_recorrentes.lift()
            self.janela_recorrentes.focus_force()
            return
        self.janela_recorrentes = JanelaTransacoesRecorrentes(self, self.banco, self.usuario, self.atualizar_tudo)

    def abrir_janela_parcelamento(self):
        if self.janela_parcelamento and self.janela_parcelamento.winfo_exists():
            self.janela_parcelamento.lift()
            self.janela_parcelamento.focus_force()
            return
        self.janela_parcelamento = JanelaParcelamentoCartao(self, self.banco, self.usuario, self.atualizar_tudo)
